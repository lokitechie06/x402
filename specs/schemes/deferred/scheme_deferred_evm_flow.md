# Deferred Scheme (EVM): Full Lifecycle Examples
...
The server forwards the payload to the facilitator for validation. The facilitator verifies per [Verification Rules](./scheme_deferred_evm.md#verification-rules-must): ERC-3009 authorization parameters, sufficient token balance for the deposit, EIP-712 voucher signer matches `channelOpen` payer (or non-zero `authorizedSigner`), channel does not already exist, and `cumulativeAmount` ≤ planned deposit. No onchain transaction occurs at this stage — funds are not yet in escrow.
...
The server returns 200 with the resource and a `PAYMENT-RESPONSE` header containing the new channel state (populated from facilitator `/settle` `extra`). The client stores `channelId` and `cumulativeAmount`for future requests.
...
> Server (off-chain): `200000 == 100000 + 100000` ✓ then forwards to facilitator.
> Facilitator (onchain rules): e.g. voucher signature, `cumulativeAmount` ≤ `channel.deposit`, `cumulativeAmount > channel.settled` (here unsettled, so typically `settled == 0`).
...
The server checks `1200000 == 1100000 + 100000` ✓, then includes its channel state in `paymentRequirements.extra`:
...
```
salt_0 = keccak256(abi.encode("x402-deferred", uint256(0)))
salt_1 = keccak256(abi.encode("x402-deferred", uint256(1)))
...
> Server (off-chain): `900000 == 800000 + 100000` ✓ then forwards to facilitator.
> Facilitator: [Verification Rules](./scheme_deferred_evm.md#verification-rules-must) — e.g. `900000` ≤ `channel.deposit`, `900000 > channel.settled` (500000).
...
After either Step 2a or 2b, the client has a fresh `PAYMENT-RESPONSE` with channel state. All subsequent requests follow the same pattern as Lifecycle 1, Step 3.