...
| `session_stale_cumulative_amount` | After channel discovery, client voucher base used onchain `settled` but server holds higher `lastCumulativeAmount` ([Resume After Discovery](#resume-after-discovery)). |
